...
void SokobanLevel::LoadMap(const char* filename)
{
	...
	// 파일 열기.
	FILE* file = nullptr;
	fopen_s(&file, path, "rt"); // 실패시 null 넘어감
	...
	// 파일에서 데이터를 읽어올 버퍼 생성.
	char* data = new char[fileSize + 1]; // \0문자로 +1 해줌

	// 데이터 읽기.
	// 실제 읽은 데이터가 위에 읽은 데이터와 같은지 확인하면 좋지만 윈도우라 \r\n에서 \r이 항상 지워지니 다른 값이라 안함. 
	...
	const Wanted::Vector2& nextPosition)
{
	...
	if (boxActor)
	{
		...
		Vector2 direction = nextPosition - playerPosition; // 빼지는 쪽의 방향으로 벡터
	...
	for (Actor* const actor : actors)
	{
		...
		// 박스나 플레이어는 아니니까 벽, 땅, 타겟 중에 하나다.
		// 근데 벽만 이동 불가한거니까 벽만 만든다.
